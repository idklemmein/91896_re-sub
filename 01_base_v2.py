...
# Display menu using pandas DataFrame
display_menu = pandas.DataFrame(kebab_dict2)
display_menu.index += 1

# Updated toppings list
topping_options = ["Hummus", "Extra Cheese", "Avocado", "Grilled Mushrooms", "Pickles",
                   "Garlic Sauce", "Chilli Sauce", "Feta Cheese", "Olives", "Tabbouleh"]
...
# Prices for kebabs (regular large)
kebab_prices = {
...
# Main routine for ordering kebabs
def order_kebab():
    order = []
    item_list = []
    quantity_list = []
    ...
    while len(order) < MAX_kebabS:
        kebab_num = num_check("Enter the number of the kebab you would like to order (1-10) or 'xxx' to finish: ", 1, 10)
        if kebab_num == "xxx":
            break
        elif kebab_num in kebab_dict:
            quantity = num_check(f"How many of {kebab_dict[kebab_num]} would you like to order? ", 1, 5)
...
def main():
    ...
    want_history = string_checker("\nWould you like to learn about the history of The Divine Kebab? ", yes_no)
    if want_history == "yes":
        history()
